...
public:
   ...
   SymbolTable(const SymbolTable &s) { tbl = new ScopeList((Scope *) NULL, s.tbl); }

   // CUSTOM: creates clone of current object with a new scope entered
   SymbolTable* clone() const { return new SymbolTable(*this); }

   // CUSTOM: convert the list of lists to a single list of ScopeEntry's
   Scope* flatten() {
     Scope* flat = (Scope*) NULL; // xxxx
     
     // loop through all scopes
     for (ScopeList* outer = tbl; outer; outer = outer->tl()) {
       // loop through all entries at scope
       for (Scope* inner = outer->hd(); inner; inner = inner->tl()) {
	 flat = new Scope(inner->hd(), flat);
       }
     }
     return flat;
   }
